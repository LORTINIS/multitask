...
Attribution. The pipeline covers:
1. Loading and preprocessing time-series sensor data.
2. Direct spike encoding that preserves temporal dynamics.
3. Training an LIF-based SNN (two hidden layers: 28 and 8 neurons) using output spike counts.
4. Computing TSA scores for individual input spikes to explain model decisions.
5. Visualising TSA as a heatmap and verifying explanations via spike ablation.

Dataset: Wine Time-Series Dataset (MQ sensor arrays + environmental signals)
Architecture: Feedforward LIF SNN with static fully-connected layers
Explainability: Gradient-based temporal attribution + spike flipping verification
"""
...
def calculate_logits_from_spikes(spike_record: torch.Tensor) -> torch.Tensor:
    """Aggregate output spikes over time to obtain logits."""
    return spike_record.sum(dim=0)
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        for spike_batch, target_batch in train_loader:
            ...
            _, spk_rec, _, _ = model(spike_batch)
            logits = calculate_logits_from_spikes(spk_rec)
...
def predict_with_spike_counts(model: TimeSeriesWineSNN, sequence: torch.Tensor,
                               device: torch.device) -> Tuple[int, torch.Tensor]:
    model.eval()
    with torch.no_grad():
        _, spk_rec, _, _ = model(sequence.unsqueeze(1).to(device))
        spike_counts = spk_rec.sum(dim=0)[0].cpu()
    predicted_class = int(torch.argmax(spike_counts).item())
    return predicted_class, spike_counts


def compute_temporal_spike_attribution(
    model: TimeSeriesWineSNN,
    ...
    """Gradient-based TSA with noise smoothing to avoid zero-grad plateaus."""
    model.eval()
    sequence = sequence.clone().detach().to(device)

    aggregated_grad = torch.zeros_like(sequence)

    for _ in range(max(num_samples, 1)):
        noisy = (sequence + noise_std * torch.randn_like(sequence)).clamp(0.0, 1.0)
        noisy.requires_grad_(True)

        _, spk_rec, _, _ = model(noisy.unsqueeze(1))
        target_spike_count = spk_rec[:, 0, target_class].sum()
        grads = torch.autograd.grad(target_spike_count, noisy, retain_graph=False)[0]
        aggregated_grad += grads.detach()

    avg_grad = aggregated_grad / max(num_samples, 1)
    attribution = (sequence * avg_grad).cpu()
    return attribution
...
def verify_attribution_by_spike_editing(
    ...
    importance = attribution.clamp(min=0.0)
    ...
    baseline_pred, baseline_counts = predict_with_spike_counts(model, original_sequence, device)
    edited_pred, edited_counts = predict_with_spike_counts(model, edited_sequence, device)
    ...
    for _ in range(random_trials):
        ...
        rand_pred, rand_counts = predict_with_spike_counts(model, random_sequence, device)
        random_outcomes.append({
            "prediction": rand_pred,
            "spike_counts": rand_counts
        })
...
for idx in range(spike_test.shape[1]):
    ...
    pred_class, _ = predict_with_spike_counts(model, sample_sequence, device)
...
pred_label, spike_counts = predict_with_spike_counts(model, sample_spike_sequence, device)

print(f"Predicted label: {label_encoder.inverse_transform([pred_label])[0]}")
print(f"Output spike counts: {spike_counts.numpy()}")

attribution = compute_temporal_spike_attribution(
    model,
...
print(f"  Baseline spike counts: {verification['baseline_counts'].numpy()}")
print(f"  Edited spike counts:   {verification['edited_counts'].numpy()}")
...
for idx, random_outcome in enumerate(verification["random_outcomes"], start=1):
    print(
        ...
        f"counts = {random_outcome['spike_counts'].numpy()}"
    )
