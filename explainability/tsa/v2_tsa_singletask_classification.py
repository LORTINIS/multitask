"""
Full TSA Explainability Script for SNN
=====================================

This script loads a trained single task SNN checkpoint and performs:
    ✅ Temporal Spike Attribution (TSA) for a selected sample
    ✅ TSA for random samples
    ✅ Aggregated TSA over the entire dataset
    ✅ Class-specific TSA comparison plots
    ✅ Perturbation curves (top-k deletion vs random deletion)

Usage:
    python tsa_singletask_classification.py --checkpoint ../../trained_models/wine_timeseries_snn_tsa_seq500_beta0.9_bs16_lr0.001_ep50.pth
    
Note: Spike data should be generated first using:
    python ../../multitask_net/utils/generate_spike_data.py
"""
...
def compute_tsa(model, x, beta, device):
    ...
    for t in range(T):
        ...
        # Create tensors on the same device as model
...
def plot_class_comparison(att_full, feature_names, class_names, save_dir=None):
    """
    att_full = [num_samples, T, F, C]
    Produces per-class average attribution heatmaps
    """
    C = att_full.shape[-1]
    for c in range(C):
...
def plot_perturbation_curve(model, x, attribution, device, feature_names, save_path=None):
    ...
    flat_imp = attribution.abs().flatten().cpu()  # Move to CPU for indexing

    # Determine available spikes (keep on CPU)
    x_cpu = x.cpu()
    ...
    for k in k_values:
        ...
        top_idx = spike_indices[top_order]  # Both on CPU now
...
def plot_perturbation_curve_multi_class(model, spike_data, labels, class_names, feature_names, 
                                        num_samples_per_class, beta, device, save_dir):
    """
    Perform perturbation analysis on multiple samples per class.
    Saves results for each sample in class-specific subdirectories.
    
    Args:
        model: The trained SNN model
        spike_data: Array of spike data samples
        labels: Array of labels
        class_names: List of class names
        feature_names: List of feature names
        num_samples_per_class: Number of samples to analyze per class
        beta: Beta parameter for TSA computation
        device: Torch device
        save_dir: Base directory for saving results
    
    Returns:
        Dictionary containing all perturbation analysis results
    """
    print(f"Performing perturbation analysis on {num_samples_per_class} samples per class...\n")
    ...
    for class_idx in range(len(class_names)):
        ...
        if len(class_sample_indices) == 0:
            ...
            all_results['classes'][class_name] = {
                'num_samples_found': 0,
                'samples': []
            }
            continue
        
        # Select random samples (up to num_samples_per_class)
        num_to_select = min(num_samples_per_class, len(class_sample_indices))
        selected_indices = np.random.choice(class_sample_indices, size=num_to_select, replace=False)
        
        print(f"  Found {len(class_sample_indices)} samples, analyzing {num_to_select} samples")
        
        # Create class-specific subdirectory
        class_dir = save_dir / f"perturbation_class_{class_name}"
        class_dir.mkdir(exist_ok=True)
        
        class_results = {
            'class_name': class_name,
            'class_idx': int(class_idx),
            'num_samples_found': len(class_sample_indices),
            'num_samples_analyzed': num_to_select,
            'samples': []
        }
        
        for i, sample_idx in enumerate(selected_indices):
            print(f"  Analyzing sample {i+1}/{num_to_select} (global index: {sample_idx})...")
            
            # Get sample data
            spikes = spike_data[sample_idx]
            ...
            # Save TSA heatmap for this sample
            heatmap_path = sample_dir / f"tsa_heatmap.png"
            plot_heatmap(att_pred, feature_names,
                        title=f"TSA - Class: {class_name}, Sample: {sample_idx} (pred: {class_names[pred]})",
                        save_path=heatmap_path)
            
            # Perform perturbation analysis
            perturbation_curve_path = sample_dir / f"perturbation_curve.png"
            perturbation_data = plot_perturbation_curve(model, x, att_pred, device, 
                                                       feature_names, save_path=perturbation_curve_path)
            ...
            sample_result = {
                ...
                'correct_prediction': bool(pred == class_idx),
                'membrane_potentials': mem.numpy().tolist(),
                'perturbation_analysis': perturbation_data,
                'save_directory': str(sample_dir.relative_to(save_dir))
            }
            
            class_results['samples'].append(sample_result)
            
            # Save individual sample summary
            sample_summary_path = sample_dir / "summary.json"
            with open(sample_summary_path, 'w') as f:
                json.dump(sample_result, f, indent=2)
            
            print(f"    Predicted: {class_names[pred]}, Correct: {pred == class_idx}")
        
        all_results['classes'][class_name] = class_results
        
        # Save class-level summary
        class_summary_path = class_dir / "class_summary.json"
        with open(class_summary_path, 'w') as f:
            json.dump(class_results, f, indent=2)
        
        print(f"  ✓ Completed analysis for class {class_name}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="TSA Explainability for SNN Wine Classification"
    )
    parser.add_argument(
        "--checkpoint", 
        type=str, 
        default="../../trained_models/wine_timeseries_snn_tsa_seq500_beta0.9_bs16_lr0.001_ep50.pth",
    ...
    parser.add_argument(
        "--full_dataset_tsa",
        action="store_true",
        help="If set, compute TSA over the entire test set for stable class-specific maps. Warning: Can be slow."
    )
    parser.add_argument(
        "--num_perturbation_samples_per_class",
        type=int,
        default=3,
        help="Number of samples per class to analyze for perturbation (default: 3)"
    )
    ...
    results_base_dir = script_dir / "results" / "classification_time_series"
    results_base_dir.mkdir(parents=True, exist_ok=True)
    
    # Create timestamped subdirectory (format: HHMM_DDMMYYYY)
    ...
    repo_root = script_dir.parent.parent  # multitask directory
    ...
    print("=" * 80)
    print("TSA EXPLAINABILITY PIPELINE")
    print("=" * 80)
    ...
    if not spike_data_dir.exists():
        raise FileNotFoundError(
            ...
            f"Please run: python {repo_root}/multitask_net/utils/generate_spike_data.py"
        )
    
    # Load both train and test spike data
    ...
    spike_train = np.load(spike_train_path)  # Shape: [samples, timesteps, features]
    spike_test = np.load(spike_test_path)  # Shape: [samples, timesteps, features]
    ...
    # Get feature and class names from config
    ...
    # Get spike sequence: [timesteps, features] - keep on CPU initially
    spikes = spike_test[sample_index]
    x = torch.tensor(spikes, dtype=torch.float32)  # Keep on CPU, will be moved to device in functions
    ...
    for i, idx in enumerate(rnd_indices):
        ...
        rnd_x = torch.tensor(rnd_spikes, dtype=torch.float32)  # Keep on CPU initially
    ...
    # -------------------------------------------------------
    # C. AGGREGATED TSA (mean attribution)
    # -------------------------------------------------------
    print("\n" + "=" * 80)
    ...
    # -------------------------------------------------------
    # D. CLASS SPECIFIC TSA COMPARISON
    # -------------------------------------------------------
    print("\n" + "=" * 80)
    ...
    if args.full_dataset_tsa:
        ...
        # Group attributions by true class
        ...
        for i in range(len(all_spike_data)):
            ...
            # Compute full attribution [T, F, C]
        ...
        # Plot average for each class
        for c_idx, att_list in class_atts.items():
            if att_list:
                # Stack tensors for this class and average over samples (dim=0)
                # Resulting shape: [T, F, C]
                avg_class_att_full = torch.stack(att_list).mean(dim=0)
                
                # Get attribution for the class itself
                avg_att_for_class_c = avg_class_att_full[:, :, c_idx]
                
                # Save class-specific TSA
    ...
    # Single sample perturbation (original functionality)
    save_path = results_dir / "perturbation_analysis_selected_sample.png"
    perturbation_data = plot_perturbation_curve(model, x, att_pred, device, feature_names, save_path=save_path)
    
    run_summary['perturbation_analysis'] = {
        'selected_sample': perturbation_data
    }

    # -------------------------------------------------------
    # F. MULTI-SAMPLE PER-CLASS PERTURBATION ANALYSIS
    # -------------------------------------------------------
    print("\n" + "=" * 80)
    print("F. MULTI-SAMPLE PER-CLASS PERTURBATION ANALYSIS")
    print("=" * 80)
    
    # Multi-sample per-class perturbation analysis
    multi_class_perturbation_results = plot_perturbation_curve_multi_class(
        model=model,
        spike_data=spike_test,
        labels=y_test,
        class_names=class_names,
        feature_names=feature_names,
        num_samples_per_class=args.num_perturbation_samples_per_class,
        beta=beta,
        device=device,
        save_dir=results_dir
    )
    
    run_summary['perturbation_analysis']['multi_sample_per_class'] = multi_class_perturbation_results

    # -------------------------------------------------------
    # G. SAVE SUMMARY JSON
    # -------------------------------------------------------
    print("\n" + "=" * 80)
    print("G. SAVING RUN SUMMARY")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("✓ ALL TSA ANALYSIS COMPLETE!")
    print(f"✓ Results saved to: {results_dir}")
    print(f"✓ Per-class perturbation results saved in class-specific subdirectories")
    print("=" * 80)
